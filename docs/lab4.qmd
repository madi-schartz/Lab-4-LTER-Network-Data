...
# Null hypothesis: Channel unit and forest type are independent 

 
# Clean data set to create contingency table 
...
salamander_table <- table(salamander_clean$section, salamander_clean$unittype)


# Chi-square assumption is that all frequencies have equal probability in the null hypothesis 

chisq.test(salamander_table)

# Results : The chi-squared test gave us the indication that there is a significant relationship between channel units and forest type since it resulted in a p-value of less than 2.2e-16. 
```
...
```{r}
# abundance distribution visually
salamander_clean |> 
  drop_na(weight_g) |> 
  ggpubr::ggviolin(x = "section", 
                   y     = "weight_g", 
                   add   = "boxplot",
                   color = "section",
                   palette = c("darkgreen", "#E7B800")) 

# Results : based on this visual we don't see too much of a difference which is why we have to conduct a statistical t-test below to confirm our assumptions. 

# Null hypothesis : Species biomass in Coastal Giant Salamander is not affected by forest type. 

# T-Test assumptions : variance of each group is equal and the data is normally distributed. 
...
# Results : Variance is not equal according to this assumption test since the p-value of 1.439e-07 is small it shows that there is a significant difference between old growth and clear cut weight. 
...
# Results : At first, the histograms showed the data as skewed to the right heavily so I added a log transform to help normalize the data. With the normalized data we see that there is a much higher p-value that supports the null. So we can use the default t.test() test which assumes equal variances, but only on a log transformed weight variable.

# t-test

t.test(log(salamander_clean$weight_g) ~ salamander_clean$section, var.equal = TRUE)

# Results : We reject the null hypothesis since we can see that the Coastal giant salamander weight was observed to be significantly higher in clear cut forests compared to old growth forests and the p-value was very small. 
...
```{r}
# Seeing which length variable represents snout to fork length

?and_vertebrates

    # length_1_mm

# Cleaning our data for a correlation test

Cutthroat_clean <- and_vertebrates |> 
  filter(species == "Cutthroat trout") |> 
  drop_na(length_1_mm, weight_g)

# Testing correlation test assumptions
# Correlation test assumption: Data are normally distributed and have a linear relationship

ggarrange(gghistogram(Cutthroat_clean$length_1_mm, title = "Length"),
gghistogram(sally_clean$weight_g, title = "Weight"))

# Results : Not normally distributed as they both look pretty skewed. We will test the normal distribution statistically using the shapiro test. 

q <- Cutthroat_clean |> 
  slice_sample(n = 5000) 

shapiro.test(q$length_1_mm)

# Results: We have a very small p-value (< 2.2e-16)

shapiro.test(q$weight_g)

# Results: We have a very small p-value (< 2.2e-16)

# RESULTS: We reject the Shapiro-Wilke null hypothesis due to our significant p-values and can conclude that our variables are not normally distributed.  

# Transforming our data to log space: 

ggarrange(
 gghistogram(log(Cutthroat_clean$length_1_mm), title = "Length"), 
 gghistogram(log(Cutthroat_clean$weight_g), title = "Weight") )

# Results: In log space our data looks bimodal. 

cor.test(log(Cutthroat_clean$length_1_mm), log(Cutthroat_clean$weight_g))

# Results: Our correlation coefficient is 0.99 and our p-value is very small (< 2.2e-16). We can conclude a very strong positive correlation between length and body mass for cutthroat trout. 
...
# The visualization confirms a very strong correlation by most of our observations falling on the line. 
...
```{r}
# Sites by Latitude
pie_crab$site <- factor(pie_crab$site, levels = pie_crab |>
                         arrange(latitude) |>
                         pull(site) |>
                         unique())  # Ensure unique levels

# Now you can perform the summarization without issues
pie_crab |>
  group_by(site) |>
...
# Findings : In doing the ANOVA test, we could see that there were significant differences in crab size across the three sites of (GTM, PIE, and DB). The results showed that there was a significant effect of site on carapace width with a p-value less than 2e-16. In addition to that test, the Tukey's HSD post-hoc test was performed. The post-hoc analysis showed that: PIE and DB, and GTM and DB varied significantly in crab size with p-values of 2.9e-06 and 3.0e-07, respectively. These results suggest that crab size varies across the sites.
```
...
```{r}
# Null hypothesis : Variation in water temperature has no effect on crab size 
...
# Results : We fail to reject the null hypothesis as this model results show a relatively large p-value (0.528), indicating that water temperature does not have a significant effect on crab size. 

# Creating a plot to visualize the linear regression model
...
```{r}
# Null hypothesis : There is no effect on crab size with the factors of latitude, variation in air temperature, and variation in water temperature

# Correlations among the three predictor variables

pie_crab |> 
  dplyr::select(latitude, air_temp_sd, water_temp_sd) |> 
  cor()

# Results : The three predictor variables of latitude, variation in air temperature, and variation in water temperature have very little correlation to one another with their correlation coefficients less than +/- 0.8. 
...
# Results : We fail to reject the null hypothesis as this model shows a low p-value of less than 2.23-16, meaning the predictor variables have a significant impact on crab size. 
```
